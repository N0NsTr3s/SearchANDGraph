    ...
    def load_from_file(self, filepath: str | Path, merge: bool = True) -> bool:
        ...
        try:
            ...
            if merge:
                ...
                # Merge graphs
                self.graph = nx.compose(self.graph, data['graph'])
                ...
                self.entity_mapping.update(data['entity_mapping'])
                
                # Merge metadata
                if 'metadata' in data:
                    self.metadata['total_pages_processed'] += data['metadata'].get('total_pages_processed', 0)
                    self.metadata['sources'].update(data['metadata'].get('sources', set()))
                ...
                self.entity_mapping = data['entity_mapping']
